...
def create_bird():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("""INSERT INTO birds (specificname, scientificname, habitat, status)
                      VALUES (%s, %s, %s, %s)""",
...
def update_bird(id):
    ...
    cursor.execute("""UPDATE birds SET specificname=%s, scientificname=%s, habitat=%s, status=%s 
                      WHERE idbirds=%s""",
